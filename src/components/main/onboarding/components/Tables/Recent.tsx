...
const Recent: React.FC<Props> = ({ filterQuery }) => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const [favUnFavSiteId, setFavUnFavSiteId] = useState('');
    const allSitesData = useSelector((state: RootState) => state.onboarding.onboardingRecentTable);
    const allSitesDataLoader = useSelector((state: RootState) => state.onboarding.onboardingRecentTableLoader);
    ...
    const columns = [
        ...
        },
        {
            ...
            title: 'Date Created',
            width: '200px',
            render: (text: any, row: any) => <div className="flex items-center justify-center text-[green]">{`${moment(text)?.format('YYYY-MM-DD  h:mm A') || text}`}</div>,
            sorter: (a: any, b: any) => moment(a.date_created).unix() - moment(b.date_created).unix(),
        },
            ...
            render: (text: any, row: any) => (
                <div className="flex items-center justify-center">
                    <div className=" w-[30px]">
                        <OnboardingActionItem rowData={row} currentTab={'recent'} filterQuery={filterQuery} />
                    </div>
                </div>
            ),
        },
