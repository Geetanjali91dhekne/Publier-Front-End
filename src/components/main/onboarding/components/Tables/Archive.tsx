...
const Archive: React.FC<Props> = ({ filterQuery }) => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const [favUnFavSiteId, setFavUnFavSiteId] = useState('');
    const allSitesData = useSelector((state: RootState) => state.onboarding.onboardingArchiveTable);
    const allSitesDataLoader = useSelector((state: RootState) => state.onboarding.onboardingArchiveTableLoader);
            ...
            render: (text: any, row: any) => (
                <div className="flex items-center justify-center">
                    <div className=" w-[30px]">
                        <OnboardingActionItem rowData={row} currentTab={'archive'} filterQuery={filterQuery} />
                    </div>
                </div>
            ),
        },
