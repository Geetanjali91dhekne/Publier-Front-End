...
const fetchprebidSiteReq = function* fetchprebidSiteReq(action: CommonAction) {
    try {
        const { data } = yield call(Apis.fetchallSiteListOnboardApi);
        const newData = prebidUploadUtils?.parsefilterData(data?.data, 'site');
        yield put(PrebidDashboardAction.setPrebidSiteList(newData || []));
    } catch (err) {
        yield put(PrebidDashboardAction.setPrebidSiteList());
        yield put(LoginActions.forbiddenTokenIssue(err || '')); // TODO If Token Expires we are redirecting to Login page...
    }
};
...
export default function* sagas() {
    ...
    yield takeLatest(PrebidDashboardTypes.FETCH_PREBID_SITE_LIST, fetchprebidSiteReq);
