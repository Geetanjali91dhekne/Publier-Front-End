...
import FilterOnSiteSize from './components/Models/filter';
...
const PreBidUpload: React.FC<Props> = ({ comparison }) => {
    ...
    const [siteAliasId, setSiteAliasId] = useState('');
    const [sizeAliasId, setSizeAliasId] = useState('');
    ...
    // const siteDataLoading = useSelector((state: RootState) => state.prebid.prebidSitelistlLoading);
    // const siteData = useSelector((state: RootState) => state.prebid.prebidSitelist);
    ...
    const buttonInputRef = useRef<any>();
    ...
    const [runApiBtnLoader, setRunApiBtnLoader] = useState(false);
    const [hideRevenue, setHideRevenue] = useState(false);
    const [validateCsv, setValidateCsv] = useState(false);
    const [validateNetwork, setValidateNetwork] = useState(false);
    const [ifCsvUploaded, setIfCsvUploaded] = useState(false);
    const [payloadData, setPayloadData] = useState<{ id: string; date: string }>({ id: '', date: '' });
    ...
    const column: any = [
        ...
        },
        {
            ...
            title: 'Revenue',
            render: (text: any) => {
                return <span>${commaSeperator(parseFloat(text || 0).toFixed(2))}</span>;
            },
        },
        {
            dataIndex: 'clicks',
    ...
    const onChangeSite = (value: string) => {
        setSiteAliasId(value);
    };

    const onChangeSize = (value: string) => {
        setSizeAliasId(value);
    };
    ...
    const onClickSubmit1 = () => {
        setRunApiBtnLoader(true);
        if (selectValue !== '' && Date !== '') {
            const payload = {
                network_id: selectValue.toString(),
                date: Date.toString(),
            };

            Apis.prebidRunApi(payload)
                .then(() => {
                    dispatch(MessageActions.showMessage({ text: 'API Executed successfully', error: false }));
                })
                .catch((err) => {
                    dispatch(MessageActions.showMessage({ text: String(err), error: true }));
                })
                .finally(() => {
                    setRunApiBtnLoader(false);
                    if (selectValue !== '' && Date !== '') {
                        const payload = {
                            id: selectValue.toString(),
                            date: Date.toString(),
    ...
    }, [Date, selectValue, siteAliasId, sizeAliasId, dispatch]);
    // console.log('testpayloadData');
    ...
    return (
        <div className=" pb-4 w-full dashboard ">
            ...
            <div className="flex justify-between">
                <div className="flex justify-between items-center gap-5">
                    ...
                    <div className="">
                        ...
                        <div className="absolute">{validateCsv && (selectValue === '' || Date === '') && <span className="common_error ml-1">Please Select Network and Date</span>}</div>
                    </div>
                        ...
                        <PButton
                            ...
                            onClick={() => {
                                if (selectValue !== '' && Date !== '') {
                                    ...
                                    setValidateNetwork(false);
                                } else {
                                    setValidateNetwork(true);
                                }
                            }}
                            loading={runApiBtnLoader}
                        />
                        <input type="button" hidden={true} ref={buttonInputRef} onClick={onClickSubmit1} />
                        <div className="absolute">{validateNetwork && (selectValue === '' || Date === '') && <span className="common_error ml-1">Please Select Network and Date</span>}</div>
                    </div>
                </div>
            ...
            <FilterOnSiteSize setFilterQuery={setFilterQuery} filterQuery={filterQuery} activeTab={activeTab} payloadData={payloadData} />
            ...
            <div className="py-4">
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {tops.map(
                        (t, index) =>
                            t?.gam === 'ON' && (
                                <PCard className="px-4 py-3 pb-2 w-full cursor-pointer topCard rounded-lg flex flex-col justify-between" key={`${index}_${t.title}`}>
            ...
            <div className="py-4">
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    <Select
                        ...
                        loading={prebidSizeAndSizeLoader}
                        onChange={onChangeSite}
                        size="large"
                        optionFilterProp="children"
                        className="w-full"
                        showSearch
                        options={prebidSiteData.map((d) => {
                            return { value: d['value'], label: d['title'] };
                        })}
                        filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
                    />
                    <Select
                        placeholder="Enter Size Alias"
                        loading={prebidSizeAndSizeLoader}
                ...
                <PModal
                    ...
                    footer={
                        <div className="flex gap-3 justify-center pb-5 items-center w-full ">
                            ...
                            <div>
                                <RoundButton title="Ok" className="w-[120px]" onClick={() => insertNewRow()} loading={insertRowLoader} />
                            </div>
                        </div>
                    }
                >
                    <div>
                        ...
                        <div className="flex justify-between">
                            <div className="w-[190px]">
                                <PNormalInput title={'Network ID'} value={selectValue} name={'network_id'} disabled={true} onChange={() => {}} />
                            </div>

                            <div className="w-[190px]">
                                <PNormalInput title={'Impressions'} value={newRowData?.impression} name={'impressions'} onChange={(e) => setNewRowData({ ...newRowData, impression: e.value })} />
                            </div>
                        </div>

                        <div className="flex justify-between">
                            <div className="w-[190px]">
                                <PNormalInput title={'Revenue'} value={newRowData?.revenue} name={'revenue'} onChange={(e) => setNewRowData({ ...newRowData, revenue: e.value })} />
                            </div>

                            <div className="w-[190px]">
                                <PNormalInput title={'Clicks'} value={newRowData?.clicks} name={'clicks'} onChange={(e) => setNewRowData({ ...newRowData, clicks: e.value })} />
                            </div>
                        </div>
                    </div>
                </PModal>
                ...
                <PModal
                    ...
                    footer={
                        <div className="flex gap-3 justify-center pb-5 items-center w-full ">
                            ...
                            <div>
                                <RoundButton title="Delete" className="w-[120px]" onClick={() => DeleteRecords()} loading={deleteRecords?.loader} />
                            </div>
                        </div>
                    }
                >
                    <div>
                        <div className="font-[Montserrat] text-[14px] text-center">
                            Enter <span className="text-[red]">'Delete'</span> to delete the records.
                        </div>
                        <div className="w-full flex justify-center">
                            <div className="w-[190px] mt-2 ">
                                <PNormalInput title={''} placeholder="Enter the text." value={deleteRecords?.deleteInput} name={'revenue'} onChange={(e) => setDeleteRecords({ ...deleteRecords, deleteInput: e.value })} />
                                {deleteRecords?.validText && deleteRecords?.deleteInput !== 'Delete' && (deleteRecords?.deleteInput === '' || deleteRecords?.deleteInput.trim() !== '') && (
                                    <span className="common_error ml-1 text-center">Enter Valid Text!</span>
                                )}
                            </div>
                        </div>
                    </div>
