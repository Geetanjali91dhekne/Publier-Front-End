...
const RecentSites: React.FC = () => {
    ...
    const loading = useSelector((state: RootState) => state.adOptDashboard.recentLoader);
    const recents = useSelector((state: RootState) => state.adOptDashboard.recents);
    ...
    return (
        <div className="py-6">
            {!loading && (
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                        {[...recents].splice(0, showAll).map((i, index) => (
                            <PCard className="rounded-lg " key={`${Math.random() * 100}`}>
                                <div className="px-4 py-4 cursor-pointer" onClick={() => redirectToDetail(i.site_id)}>
                                    <div>
                                        <div className="flex justify-between items-center">
                                            <p className="text-xs color-BDBDBD roboto-medium">Revenue</p>
                                            {/* <p className="text-xs roboto-medium p-1 color-BDBDBD">{moment(i.date).format('MM/DD/YYYY')}</p> */}
                                        </div>
            ...
            {!loading && recents.length === 0 && <Empty description="No recents data" />}
        </div>
    );
};
