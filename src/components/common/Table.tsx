...
const PTable = <ObjectType extends {}>({ columns, data, isHideSort, loading, actions, pagination, rowKey, onRowClick, summary, rowSelection, className }: Props<ObjectType>) => {
            ...
            <div className={`table-row-dark ${className}`}>
                <Table
                    ...
                    pagination={
                        ...
                        pagination?.isShow ? pagination.customePagination || { position: ['bottomRight'],style:{marginRight:'15px'}, showTotal: (total, range) => `${range[0]}-${range[1]} of ${total}`, showSizeChanger: true, pageSizeOptions: [10, 20, 50, 100] } : false
                    }
                    summary={summary}
